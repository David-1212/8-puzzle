...
def dls(inicial, meta):
    ...
    while frontera:                         #Mientras haya nodos por explorar:
        nodo = frontera.pop()           #Se toma el primer nodo de la cola.
        ...
        if nodo.estado == meta:                         #Si es una meta, 
            ...
            if nodo.profundidad < limite:
                frontera.extend(nodo.encontrar_sucesores(meta))

    # best.imprimir_nodo()
    # print(best.piezas_correctas)
    return best.encontrar_camino(), costo_camino - 1

# Algoritmo Iterative deepening depth-first search
def ids(inicial, meta, limite):
    print('\nLimite:', limite)
...
def a_star(inicial, meta):
    ...
    while not frontera.empty():                         #Mientras haya nodos por explorar:
...
if __name__ == '__main__':
    ...
    # print('----- DLS -----')
    # nodo_inicial = Nodo(estado_inicial, None, None, 0, calcular_heurisitica(estado_inicial, estado_final))
    # nodos_camino, costo_camino = dls(estado_inicial, estado_final)

    # -------------------- IDS --------------------
    # print('----- IDS -----')
    # nodo_inicial = Nodo(estado_inicial, None, None, 0, calcular_heurisitica(estado_inicial, estado_final))
    # nodos_camino, costo_camino = ids(estado_inicial, estado_final, limite)

    # while not nodos_camino:
    #     limite = limite + 1
    #     nodos_camino, costo_camino = ids(estado_inicial, estado_final, limite)
    ...
    # print('----- A* -----')
    # nodo_inicial = Nodo(estado_inicial, None, None, 0, calcular_heurisitica(estado_inicial, estado_final))
    # nodos_camino, costo_camino = a_star(estado_inicial, estado_final)
